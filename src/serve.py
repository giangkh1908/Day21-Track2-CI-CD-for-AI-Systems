...
def download_model():
    ...
    Ham nay duoc goi mot lan khi module duoc import. boto3 su dung
    AWS credentials tu IAM Role, bien moi truong, hoac ~/.aws/credentials.
    """
    os.makedirs(os.path.dirname(MODEL_PATH), exist_ok=True)
    s3 = boto3.client("s3")
    s3.download_file(S3_BUCKET, S3_MODEL_KEY, MODEL_PATH)
    print(f"Model da duoc tai tu s3://{S3_BUCKET}/{S3_MODEL_KEY}")
